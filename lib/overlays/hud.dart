...
class Hud extends PositionComponent with HasGameReference<EmberQuestGame> {
  ...
  Future<void> onLoad() async {
    _scoreTextComponent = TextComponent(
      ...
      position: Vector2(game.size.x - 60, game.isMobile ? 60 : 20),
    );
    ...
    add(
      SpriteComponent(
        sprite: starSprite,
        position: Vector2(game.size.x - 100, game.isMobile ? 60 : 20),
        size: Vector2.all(32),
        anchor: Anchor.center,
      ),
    );
    ...
    for (var i = 1; i <= game.health; i++) {
      ...
      await add(
        HeartHealthComponent(
          heartNumber: i,
          position: Vector2(positionX.toDouble(), game.isMobile ? 50 : 20),
          size: Vector2.all(32),
        ),
      );
    }
